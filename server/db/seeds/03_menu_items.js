/**
 * @param { import("knex").Knex } knex
 * @returns { Promise<void> }
 */
exports.seed = async function (knex) {
  // Deletes ALL existing entries
  await knex('menu_items').insert([
    {
      id: 1,
      name: 'Vegetarian Mini Spring Rolls ( 10 pcs )',
      description: "They ain't summer rolls!",
      price: 12,
      stock: 40,
      image_url: 'vegetarian-mini-spring-rolls.jpeg',
      category_id: 1,
      active: false,
    },
    {
      id: 2,
      name: 'Vegetarian Mini Samosas ( 10 pcs )',
      description: '',
      price: 12,
      stock: 40,
      image_url: 'vegetarian-mini-samosas.jpeg',
      category_id: 1,
      active: true,
    },
    {
      id: 3,
      name: 'Deep Fried Vegetarian Dumplings ( 10 pcs )',
      description: '',
      price: 13,
      stock: 40,
      image_url: 'deep-fried-dumplings.jpeg',
      category_id: 1,
      active: true,
    },
    {
      id: 4,
      name: 'Thai Fried Rice',
      description:
        'Thai style stir fried rice with Choice of Meat and Vegetables',
      price: 18.8,
      stock: 40,
      image_url: 'thai-fried-rice.jpeg',
      category_id: 2,
      active: true,
    },
    {
      id: 5,
      name: 'Fried Rice',
      description: '',
      price: 18.8,
      stock: 40,
      image_url: 'fried-rice.jpeg',
      category_id: 2,
      active: true,
    },
    {
      id: 6,
      name: 'Nasi Goreng',
      description:
        'Stir fried choice of meat, rice, mixed vegetables and sambal chilli.',
      price: 18.8,
      stock: 20,
      image_url: 'nasi-goreng.jpeg',
      category_id: 2,
      active: true,
    },
    {
      id: 7,
      name: 'Nasi Goreng + Fried Egg',
      description:
        'Stir fried choice of meat with egg, rice, mixed vegetables and sambal chilli.',
      price: 20.8,
      stock: 20,
      image_url: 'nasi-goreng-fried-egg.jpeg',
      category_id: 2,
      active: true,
    },
    {
      id: 8,
      name: 'Prawns Fried Rice',
      description: '',
      price: 21.8,
      stock: 25,
      image_url: 'prawns-fried-rice.jpeg',
      category_id: 2,
      active: true,
    },
    {
      id: 9,
      name: 'Duck Fried Rice',
      description: '',
      price: 21.8,
      stock: 20,
      image_url: 'duck-fried-rice.jpeg',
      category_id: 2,
      active: true,
    },
    {
      id: 10,
      name: 'Crispy Pork Fried Rice',
      description: '',
      price: 21.8,
      stock: 30,
      image_url: 'crispy-pork-fried-rice.jpeg',
      category_id: 2,
      active: true,
    },
    {
      id: 11,
      name: 'Spicy Fried Rice',
      description: '',
      price: 18.8,
      stock: 20,
      image_url: 'spicy-fried-rice.jpeg',
      category_id: 2,
      active: true,
    },
    {
      id: 12,
      name: 'Pineapple Fried Rice',
      description:
        'Stir fried choice of meat, egg, rice, vegetables, pineapple and cashew nuts',
      price: 19.8,
      stock: 20,
      image_url: 'pineapple-fried-rice.jpeg',
      category_id: 2,
      active: true,
    },
    {
      id: 13,
      name: 'Tom Yum Fried Rice',
      description: '',
      price: 18.8,
      stock: 20,
      image_url: 'tom-yum-fried-rice.jpeg',
      category_id: 2,
      active: true,
    },
    {
      id: 14,
      name: 'Fried Rice With Cashew Nuts',
      description: '',
      price: 19.8,
      stock: 20,
      image_url: 'fried-rice-with-cashew-nuts.jpeg',
      category_id: 2,
      active: true,
    },
    {
      id: 15,
      name: 'Vegetarian Fried Rice',
      description: 'Stir fried rice, mixed vegetables, soya sauce without egg.',
      price: 18.8,
      stock: 20,
      image_url: 'vegetarian-fried-rice.jpeg',
      category_id: 2,
      active: true,
    },
    {
      id: 16,
      name: 'Tofu Fried Rice',
      description:
        'Stir fried tofu with egg, rice, mixed vegetables and soya sauce.',
      price: 18.8,
      stock: 20,
      image_url: 'tofu-fried-rice.jpeg',
      category_id: 2,
      active: true,
    },
    {
      id: 17,
      name: 'Pad Thai',
      description: '',
      price: 18.8,
      stock: 40,
      image_url: 'pad-thai.jpeg',
      category_id: 3,
      active: true,
    },
    {
      id: 18,
      name: 'Pad Thai Duck',
      description: '',
      price: 21.8,
      stock: 40,
      image_url: 'pad-thai-duck.jpeg',
      category_id: 3,
      active: true,
    },
    {
      id: 19,
      name: 'Kuey Teow Goreng Duck',
      description: '',
      price: 21.8,
      stock: 40,
      image_url: 'kuey-teow-goreng-duck.jpeg',
      category_id: 3,
      active: true,
    },
    {
      id: 20,
      name: 'Chow Mein',
      description: 'Yellow noodle and vegetables.',
      price: 18.8,
      stock: 40,
      image_url: 'chow-mein.jpeg',
      category_id: 3,
      active: true,
    },
    {
      id: 21,
      name: 'Singapore Noodles',
      description: '',
      price: 18.8,
      stock: 40,
      image_url: 'singapore-noodles.jpeg',
      category_id: 3,
      active: true,
    },
    {
      id: 22,
      name: 'Mee Goreng',
      description: 'Yellow noodles, egg, carrot, cabbage and bean sprouts.',
      price: 18.8,
      stock: 40,
      image_url: 'mee-goreng.jpeg',
      category_id: 3,
      active: true,
    },
    {
      id: 23,
      name: 'Tom Yum Fried Noodles',
      description: 'Yellow noodles, tom yum flavour and vegetables.',
      price: 18.8,
      stock: 40,
      image_url: 'tom-yum-fried-noodles.jpeg',
      category_id: 3,
      active: true,
    },
    {
      id: 24,
      name: 'Satay Noodles',
      description:
        'Yellow noodles, vegetables, peanuts sauce and your choice of meat',
      price: 18.8,
      stock: 40,
      image_url: 'satay-noodles.jpeg',
      category_id: 3,
      active: true,
    },
    {
      id: 25,
      name: 'Cashew Nuts Noodles',
      description: 'Yellow noodles, chilli paste and vegetables.',
      price: 19.8,
      stock: 40,
      image_url: 'cashew-nuts-noodles.jpeg',
      category_id: 3,
      active: true,
    },
    {
      id: 26,
      name: 'Drunken Noodles (Pad Kee Mao)',
      description:
        'Stir fried rice noodles with choice of meat, chilli, garlic and basil leaves.',
      price: 18.8,
      stock: 40,
      image_url: 'drunken-noodles.jpeg',
      category_id: 3,
      active: true,
    },
    {
      id: 27,
      name: 'Char Kuey Teow',
      description:
        'Flat rice noodles, egg, carrot, cabbage, bean sprouts with sambal chilli',
      price: 18.8,
      stock: 40,
      image_url: 'char-kuey-teow.jpeg',
      category_id: 3,
      active: true,
    },
    {
      id: 28,
      name: 'Pad See Ew',
      description: 'Flat rice noodles, dark soy sauce, egg, vegetables',
      price: 18.8,
      stock: 40,
      image_url: 'pad-see-ew.jpeg',
      category_id: 3,
      active: true,
    },
    {
      id: 29,
      name: 'Fried Kuey Teow',
      description: 'Flat rice noodles, egg, carrot, cabbage and bean sprouts.',
      price: 18.8,
      stock: 40,
      image_url: 'fried-kuey-teow.jpeg',
      category_id: 3,
      active: true,
    },
    {
      id: 30,
      name: 'Kuey Teow Goreng',
      description: '',
      price: 18.8,
      stock: 40,
      image_url: 'kuey-teow-goreng.jpeg',
      category_id: 3,
      active: true,
    },
    {
      id: 31,
      name: 'Coke',
      description: 'Non-Alcoholic',
      price: 4,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 32,
      name: 'Diet Coke',
      description: 'Non-Alcoholic',
      price: 4,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 33,
      name: 'Coke Zero',
      description: 'Non-Alcoholic',
      price: 4,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 34,
      name: 'Sprite',
      description: 'Non-Alcoholic',
      price: 4,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 35,
      name: 'Fanta Orange',
      description: 'Non-Alcoholic',
      price: 4,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 36,
      name: 'Raspbery Coke',
      description: 'Non-Alcoholic',
      price: 4.5,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 37,
      name: 'Raspbery Lemonade',
      description: 'Non-Alcoholic',
      price: 4.5,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 38,
      name: 'Ginger Beer',
      description: 'Non-Alcoholic',
      price: 5.5,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 39,
      name: 'Lemon Lime Bitters',
      description: 'Non-Alcoholic',
      price: 5.5,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 40,
      name: 'Thai Mike Tea',
      description: 'Non-Alcoholic',
      price: 6,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 41,
      name: 'Coconut Water',
      description: 'Non-Alcoholic',
      price: 6,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 42,
      name: 'Mocktail',
      description: 'Non-Alcoholic',
      price: 8,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 43,
      name: 'Orange Juice',
      description: 'Juice',
      price: 5,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 44,
      name: 'Apple Juice',
      description: 'Juice',
      price: 5,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 45,
      name: 'Pineapple Juice',
      description: 'Juice',
      price: 5,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 46,
      name: 'Peach Soju',
      description: 'Soju',
      price: 14,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 47,
      name: 'GreenGrape Soju',
      description: 'Soju',
      price: 14,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 48,
      name: 'Strawberry Soju',
      description: 'Soju',
      price: 14,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 49,
      name: 'Watermelon Soju',
      description: 'Soju',
      price: 14,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 50,
      name: 'Pineapple Soju',
      description: 'Soju',
      price: 14,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 51,
      name: 'Singha',
      description: 'Beer',
      price: 9,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 52,
      name: 'Chang',
      description: 'Beer',
      price: 9,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 53,
      name: 'Heineken Light',
      description: 'Beer',
      price: 9,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 54,
      name: 'Heineken',
      description: 'Beer',
      price: 9,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 55,
      name: 'Tiger',
      description: 'Beer',
      price: 9,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 56,
      name: 'Asahi',
      description: 'Beer',
      price: 9,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 57,
      name: 'Corona',
      description: 'Beer',
      price: 9,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 58,
      name: 'Merlot Cabernet',
      description: 'Red Wines',
      price: 9,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 59,
      name: 'Shiraz',
      description: 'Red Wines',
      price: 9,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 60,
      name: 'Piont Noir',
      description: 'Red Wines',
      price: 9,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 61,
      name: 'Sauvignon Blanc',
      description: 'White Wines',
      price: 9,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 62,
      name: 'Chardonnay',
      description: 'White Wines',
      price: 9,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
    {
      id: 63,
      name: 'Pinor Gris',
      description: 'White Wines',
      price: 9,
      stock: 40,
      image_url: '',
      category_id: 4,
      active: true,
    },
  ])
}
